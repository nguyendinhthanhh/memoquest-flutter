...
void main() {
  ...
  testWidgets('full MemoQuest user flow passes on a fresh install', (
    tester,
  ) async {
    ...
    expect(find.text('ÄÄƒng nháº­p MemoQuest'), findsOneWidget);
    ...
    await tapText(tester, 'ÄÄƒng nháº­p');
    expect(find.text('Home'), findsNothing);
    expect(find.textContaining('Sai email'), findsOneWidget);
    ...
    expect(find.text('Xin chÃ o!'), findsOneWidget);
    ...
    await tapText(tester, 'LÆ°u ghi chÃº');

    expect(find.text(createdTitle), findsOneWidget);

    logStep('6 edit note');
    await tapText(tester, createdTitle);
    expect(find.text('Chi tiáº¿t Note'), findsOneWidget);
    ...
    logStep('7 pin note');
    final updatedTitleFinder = find.text(editedTitle);
    expect(updatedTitleFinder, findsOneWidget);
    ...
    await tapText(tester, 'Generate Flashcards');
    expect(find.text('LÆ°u táº¥t cáº£'), findsOneWidget);
    await tapText(tester, 'LÆ°u táº¥t cáº£');
    expect(find.text('Chi tiáº¿t Note'), findsOneWidget);

    logStep('12 create flashcard manually');
    await tapText(tester, 'Xem Flashcards');
    ...
    await tapText(tester, 'LÆ°u flashcard');
    ...
    expect(find.textContaining('HoÃ n táº¥t review'), findsOneWidget);
    ...
    for (var index = 0; index < cardsForQuiz.length; index++) {
      final questionTextWidget = tester.widget<Text>(
        find.descendant(
    ...
    expect(find.text('${cardsForQuiz.length * 10}'), findsOneWidget);
    ...
    expect(find.text('Streak: ${progressAfterQuiz.streak} ngÃ y'), findsOneWidget);
    ...
    expect(find.text('Home'), findsOneWidget);
    final materialAppAfterRestart = tester.widget<MaterialApp>(
      find.byType(MaterialApp),
    );
    expect(materialAppAfterRestart.themeMode, ThemeMode.dark);
    ...
    await tapText(tester, 'XÃ¡c nháº­n');
    expect(find.textContaining('ÄÃ£ reset'), findsOneWidget);

    await tapText(tester, 'Home');
    expect(find.text('0'), findsWidgets);
    await tapText(tester, 'Notes');
    expect(find.textContaining('ChÆ°a cÃ³ ghi chÃº'), findsOneWidget);
    await tapText(tester, 'Cards');
    expect(find.textContaining('ChÆ°a cÃ³ flashcards'), findsOneWidget);
    await tapText(tester, 'Stats');
    expect(find.text('XP: 0'), findsOneWidget);
    expect(find.text('Streak: 0 ngÃ y'), findsOneWidget);
